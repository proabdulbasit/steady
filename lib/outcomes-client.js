...
/** Local testing — send due follow-up emails now (does not wait for the daily hour). */
export async function sendDueOutcomeEmails({ authToken }) {
  const response = await fetch(getBackendUrl("/api/outcomes/send-due-emails"), {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...withAuth(authToken),
...
export function buildPartialFollowUpPrompt(outcome, happened = "") {
  ...
  const what = String(happened || "").trim();
  return `${ask}That advice only partly worked.

Advice was: "${advice}"

Here's what happened: ${what || "[tell Steady what worked and what didn't]"}

Please adjust the plan with clearer steps for my team.`;
}

export function buildDidntWorkFollowUpPrompt(outcome, happened = "") {
  const advice = outcome?.adviceExcerpt || "the last plan Steady gave me";
  const ask = outcome?.userPromptExcerpt ? `Original problem: ${outcome.userPromptExcerpt}\n` : "";
  const what = String(happened || "").trim();
  return `${ask}That advice didn't work.

Advice was: "${advice}"

Here's what happened: ${what || "[tell Steady what you tried and what went wrong]"}

Please give a different plan with clearer steps for my team.`;
}
